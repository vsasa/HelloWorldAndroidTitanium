...
@implementation hello_world_android__hello_world_android_titaniumObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_hello_world_android_titanium);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any hello_world_android__hello_world_android_titanium patches in JS before a context is loaded... nice for 
...
{
	[context gc];
	[_hello_world_android_titanium gc];
}
...
{
	// create hello_world_android__hello_world_android_titanium global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_hello_world_android_titanium = [[hello_world_android__hello_world_android_titaniumObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hello_world_android_titanium];
	
	NSString *_hello_world_android_titaniumNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _hello_world_android_titaniumNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hello_world_android_titanium valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.hello_world_android_titanium.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
